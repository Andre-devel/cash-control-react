...
import { resetInstallmentsStore } from '@/test/handlers/installments.handlers'
...
function renderDialog(open = true) {
  const onClose = vi.fn()
  renderWithProviders(<CreateInstallmentDialog open={open} onClose={onClose} />)
  return { onClose }
}
...
describe('CreateInstallmentDialog — payment method integration', () => {
  ...
  it('blocks submit when CREDIT_CARD is selected but no card is chosen', async () => {
    ...
    await user.type(screen.getByRole('textbox', { name: /descrição/i }), 'Laptop parcelado')
  ...
  it('switching from CREDIT_CARD to another method hides the picker', async () => {
    const user = userEvent.setup()
    renderDialog()
    await waitForPaymentMethodsLoaded()
